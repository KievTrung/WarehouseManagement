...
import com.KievTrung.core.service.*;
...
public class MainForm extends JFrame {
    ...
    private JTable tableCollectionDetail;
...
//    }
    ...
    public static void loadTable(JTable table, String[] col, List<Object[]> data, int selectionMode) {
    ...
    class PersonHandler implements Handler<Person> {
        ...
        @Override
        public void getAll() {
            loadTable(personTable, personCol, prepareTableRow(personRepository.findAll(type)), ListSelectionModel.SINGLE_SELECTION);
        }
        ...
        public void find() {
            ...
            loadTable(personTable, personCol, prepareTableRow(personRepository.findByName(type, name)), ListSelectionModel.SINGLE_SELECTION);
        }
    ...
    class ItemHandler implements Handler<Item> {
        ...
        public void getAll() {
            ...
            loadTable(tableItem, itemCol, objects, ListSelectionModel.SINGLE_SELECTION);
        }
    ...
    class VoucherHandler implements Handler<Voucher> {
            ...
            tableVoucher.addMouseListener(new MouseAdapter() {
                ...
                public void mouseClicked(MouseEvent e) {
                    ...
                    if (row >= 0) {
                        ...
                        // table voucher line
                        loadTable(tableVoucherLine, voucherLineCol, prepareDetailTableRow(voucherService.findAllLineById(type, voucherId)), ListSelectionModel.SINGLE_SELECTION);
        ...
        public void getAll() {
            ...
            loadTable(type == Person.Type.CUSTOMER ? tableSaleVoucher : tablePurchaseVoucher, voucherCol, prepareTableRow(vouchers), ListSelectionModel.SINGLE_SELECTION);
        }
    ...
    class PaymentHandler implements Handler<Payment> {
        ...
        public PaymentHandler(Person.Type type) {
            ...
            jTable = type == Person.Type.CUSTOMER ? tableCollection : tablePayment;
            jTable.addMouseListener(new MouseAdapter() {
                ...
                public void mouseClicked(MouseEvent e) {
                    ...
                    if (row >= 0) {
                        ...
                        loadTable(tablePaymentDetail, paymentDetailCol, prepareDetailTableRow(paymentDetailRepository.findAllById(type, paymentId)), ListSelectionModel.SINGLE_SELECTION);
                    }
                }
            });
        }
...
//                    FormatDateTime.toStr(o.getCreateDate()),
        ...
        @Override
        public void getAll() {
            loadTable(tablePayment, paymentCol, prepareTableRow(paymentService.getAll(type)), ListSelectionModel.SINGLE_SELECTION);
        }
    ...
    class DebtHandler implements Handler<Debt> {
        ...
        public DebtHandler(Person.Type type) {
            ...
            jTable = type == Person.Type.CUSTOMER ? tableCustomerDebt : tableSupplierDebt;

            // update multiple interest
            jTable.getSelectionModel().addListSelectionListener(e -> {
                // Avoid duplicate events while adjusting
                if (!e.getValueIsAdjusting()) {

                    List<Integer> voucherIds = Arrays
                            .stream(jTable.getSelectedRows())
                            .mapToObj(row -> {
                                int voucherId = Integer.parseInt(jTable.getValueAt(row, 0).toString());
                                log.debug("selectd voucher: {}", voucherId);
                                return voucherId;
                            }).toList();

                    debtService.executeUpdateInterest(type, voucherIds);
                    getAll();
                }
            });
        }
        ...
        @Override
        public void getAll() {
            loadTable(jTable, debtCol, prepareTableRow(debtRepository.findAll(type)), ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
        }
